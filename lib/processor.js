...
module.exports = function(db,logger,config) {
    ...
    var masterKey = new bitcore.HDPrivateKey.fromSeed( bitcore.crypto.Hash.sha256(new Buffer.from([config.seedBytes])) ,config.network);
    ...
    function sendTX(unspent,to,privKeys,callback){
        ...
        request.post({url:url, form: {"hex":raw_tx}}, function(err,httpResponse,hash){
            if (err){
                ...
                logger.log('Success sending raw tx: ', [raw_tx]);
                callback(null,true);
            } else{
                logger.log('Probelm sending raw tx: ', [raw_tx]);
                callback(null,false);
            }
        });
    ...
    module.update = function(callback){
        if (status != 'stop'){
            ...
            //All addresses waiting checked   
                ...
                if (err){
                    logger.error('Unable to update address', [err.message]);
                    if (callback)
                        callback(err);
                    else
                        throw err;
    ...
    */
    module.getFreeAddress = function(newPaymentWaiting,callback){
        async.waterfall([
            function(callback){
                db.poolAddresses.findOne({'waiting': false},{},function(err,address){
                    if (err){
            ...
            if (err){
                logger.error("Error getting free address from pool", [err.message])
                callback(err);
            } else {
               callback(null,address);
            }
    ...
    module.getAddressesWaiting = function(callback){
        db.poolAddresses.find({'waiting': true},{},function(err, addressesWaiting){
            if (err)
                callback(err);
    ...
    function checkAddressWaiting(addressW,callback){
        ...
        async.waterfall([
            ...
            function(paymentW,callback){
                logger.log('Getting utxos..', [paymentW]);
            ...
            function(paymentW,txs,amountW,unusedBalance,unspent,fromAddress,callback){ 
                if ((amountW >= paymentW.quantity)&&(unusedBalance >= config.limitBalance)&&(unspent.length > 0)){
                    ...
                    if ((config.warningTimeout > 0) && paymentW && (!paymentW.warned) && (unspent.length == 0) && ( (((new Date(paymentW.dateBegin.getTime() + config.warningTimeout*60000))-new Date())/60000) < 0)){                      
                        paymentW.warn(function(err,done){
                            if (err){
                                callback(err);
                            } else {
                                logger.log('PaymentW warning..');
                                callback(null,true,0);
                            }
                        });
                    } else if (paymentW && (unspent.length == 0) && ( (((new Date(paymentW.dateBegin.getTime() + config.paymentTimeout*60000))-new Date())/60000) < 0)){
                        paymentW.remove();
            ...
            function(finishWaiting,unusedBalance,callback){
                if (finishWaiting){
                    logger.log('Address finishing waiting..', [finishWaiting, unusedBalance]);
